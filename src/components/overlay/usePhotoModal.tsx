...
export function usePhotoModal({ id, images, photos }: PhotoModalProps): PhotoModalInterface {
    ...
    const togglePhoto = useCallback((image: string) => {
    ...
    const generateTriggerFigure = useCallback(({image, className}: {image: string, className?: string}) => (
        <figure
            key={image}
            className={className}
    ...
    const modalBody = (
        <>
            ...
            <img src={photos[currentPhotoKey].src} alt={photos[currentPhotoKey].alt} className={photos[currentPhotoKey].className} />
            <button onClick={() => setImageIndex((prev) => prev + 1)} disabled={imageIndex === imagesCount - 1}>
                <span className="material-icons">
                    chevron_right
                </span>
            </button>
    ...
    const photoHook = useOverlay({
        ...
        content:{
            header: photos[currentPhotoKey].caption,
            body: modalBody
        },
